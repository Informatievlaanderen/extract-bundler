...
public sealed class ExtractDownloader
{
    ...
    public ExtractDownloader(
        ...
        ILogger<ExtractDownloader>? logger)
    {
    ...
    public event EventHandler OnAllZipArchivesDownloaded;
    ...
    private async Task Download(string url, string fileName, CancellationToken cancellationToken = default)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            zipArchives.Add(fileName, archive);
            OnZipArchiveDownloaded?.Invoke((fileName, archive), null);
            return;
        }

        OnZipArchiveDownloadFailed?.Invoke($"{fileName} : {url}", null);
    }

    private async Task<ZipArchive> GetZipArchiveWithDownloadProgressAsync(HttpResponseMessage response,string fileName,
        CancellationToken cancellationToken = default)
    {
        ...
        do
        {
            ...
            {
                isMoreToRead = false;
                Console.WriteLine($"Download Complete {fileName}.zip ({totalBytesRead.FormatBytes()})");
                continue;
            }
            ...
            if (readCount % 1000 == 0)
            {
                Console.WriteLine($"Download {fileName}.zip ({totalBytesRead.FormatBytes()})");
            }

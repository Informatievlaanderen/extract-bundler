...
public sealed class MetaDataCenterHttpClient
{
        ...
        ILogger<MetaDataCenterHttpClient>? logger)
    {
        ...
        DefaultHeaders.Add("Authorization", new string[] { $"Basic {_options.Authorization}".Trim() });
    }
...
<csw:Transaction service=""CSW"" version=""2.0.2""
	...
	<csw:Update>
        <csw:RecordProperty>
            ...
            <csw:Value>{dateStamp.ToString("yyyy-MM-dd")}</csw:Value>
        </csw:RecordProperty>
        <csw:RecordProperty>
...
"));
    ...
    {
        var requestMessage =
            new HttpRequestMessage(HttpMethod.Post, new Uri(_baseUrl, "/metadatacenter/srv/dut/info?info=me"));
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(xsrfToken))
            {
                ...
                throw new ArgumentNullException("Unable to retrieve XSRF-TOKEN");
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<XDocument> UpdateCswPublication(Identifier identifier, DateTime dateStamp,
        CancellationToken cancellationToken = default)
    {
        string xsrfToken = await GetXsrfToken(cancellationToken);
        var requestMessage =
            new HttpRequestMessage(HttpMethod.Post, new Uri(_baseUrl, "/metadatacenter/srv/dut/csw-publication"));
        foreach (var header in DefaultHeaders)
        {
            requestMessage.Headers.TryAddWithoutValidation(header.Key, header.Value);
        }
    ...
    public async Task<string> GetXmlAsString(Identifier identifier, CancellationToken cancellationToken = default)
    {
        var requestMessage =
            new HttpRequestMessage(HttpMethod.Get,
                new Uri(_baseUrl, $"/metadatacenter/srv/api/records/{GetIdentifierValue(identifier)}/formatters/xml"));
        foreach (var header in DefaultHeaders)
        {
            requestMessage.Headers.TryAddWithoutValidation(header.Key, header.Value);
        }
    ...
    public async Task<byte[]> GetPdfAsByteArray(Identifier identifier, CancellationToken cancellationToken = default)
    {
        var requestMessage =
            new HttpRequestMessage(HttpMethod.Get,
                new Uri(_baseUrl,
                    $"/metadatacenter/srv/api/records/{GetIdentifierValue(identifier)}/formatters/xsl-view-default?output=pdf&language=dut&approved=true"));
        foreach (var header in DefaultHeaders)
        {
            requestMessage.Headers.TryAddWithoutValidation(header.Key, header.Value);
        }
